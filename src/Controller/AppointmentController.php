...
final class AppointmentController extends AbstractController
{
    ...
    #[Route('/my', name: 'app_my_appointments', methods: ['GET'])]
    public function myAppointments(AppointmentService $appointmentService): Response
    {
        $this->denyAccessUnlessGranted('ROLE_USER');

        /** @var \App\Entity\User $user */
        $user = $this->getUser();
        $appointments = $appointmentService->findAppointmentsForUser($user);
    ...
    #[Route('/doctor', name: 'app_doctor_appointments', methods: ['GET'])]
    public function doctorAppointments(AppointmentService $appointmentService): Response
    {
        $this->denyAccessUnlessGranted('ROLE_USER');

        /** @var \App\Entity\User $user */
        $user = $this->getUser();
        $appointments = $appointmentService->findAppointmentsForDoctorUser($user);

        return $this->render('appointment/doctor_appointments.html.twig', [
            'appointments' => $appointments,
        ]);
    }
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, #[CurrentUser] $user): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            // If a user is logged in, associate and prefill patient data
            if ($user) {
